...
#[tauri::command]pub fn save_settings(app:AppHandle,settings:AppSettings)->Result<AppSettings,String>{
    ...
    let old=state.settings.read().clone(); *state.settings.write()=next.clone();
    if let Err(e)=shortcuts::register_all(&app){*state.settings.write()=old.clone();let _=shortcuts::register_all(&app);return Err(e)}
    let startup_result=if next.general.launch_at_startup{app.autolaunch().enable()}else{app.autolaunch().disable()};
    if let Err(e)=startup_result{*state.settings.write()=old;let _=shortcuts::register_all(&app);return Err(e.to_string())}
    if let Some(tray)=app.tray_by_id("main-tray"){tray.set_visible(next.general.show_tray_icon).map_err(|e|e.to_string())?;}
    state.history.lock().truncate(next.history.max_items); settings_store::save_settings(&app,&next)?;
    let items=state.history.lock().items(); settings_store::save_history(&app,next.history.persist_history,&items)?; Ok(next)
}
...
#[tauri::command]pub fn toggle_monitoring(app:AppHandle)->Result<bool,String>{let state=app.state::<AppState>();let value={let mut s=state.settings.write();s.general.monitoring_paused=!s.general.monitoring_paused;settings_store::save_settings(&app,&s)?;s.general.monitoring_paused};Ok(value)}
...
#[tauri::command]pub fn complete_first_run(app:AppHandle)->Result<AppSettings,String>{let state=app.state::<AppState>();let mut s=state.settings.write();s.first_run_completed=true;settings_store::save_settings(&app,&s)?;Ok(s.clone())}
#[tauri::command]pub fn reset_settings(app:AppHandle)->Result<AppSettings,String>{let defaults=AppSettings::default().normalized();let state=app.state::<AppState>();*state.settings.write()=defaults.clone();shortcuts::register_all(&app)?;app.autolaunch().disable().map_err(|e|e.to_string())?;if let Some(tray)=app.tray_by_id("main-tray"){let _=tray.set_visible(defaults.general.show_tray_icon);}settings_store::save_settings(&app,&defaults)?;settings_store::save_history(&app,false,&[])?;Ok(defaults)}
