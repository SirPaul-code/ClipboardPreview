...
fn action(app:&AppHandle,shortcut:&Shortcut,state:ShortcutState){
    ...
    if q.as_ref()==Some(shortcut)&&matches!(state,ShortcutState::Pressed){let _=overlays::show_quick(app);}
    else if h.as_ref()==Some(shortcut){match state{ShortcutState::Pressed=>{if !app.state::<AppState>().selector.lock().active{let _=overlays::begin(app,settings.history.interaction_mode.clone());}},ShortcutState::Released=>{if matches!(settings.history.interaction_mode,InteractionMode::HoldRelease){let _=selection::accept(app);}}}}
    else if o.as_ref()==Some(shortcut)&&matches!(state,ShortcutState::Pressed){let _=overlays::open_settings(app);}
    else if p.as_ref()==Some(shortcut)&&matches!(state,ShortcutState::Pressed){let mut s=app.state::<AppState>().settings.write();s.general.monitoring_paused=!s.general.monitoring_paused;let _=crate::settings_store::save_settings(app,&s);}
}
